...
# Scenarios setup ----
N_reps <- 200
scenario <- expand.grid(nGroupT = c(2, 4, 6), nGroupC = 2, meanC = 3.46, 
                        sdC = c(0.09, 0.2, 0.5),
                        ef = c(0.2), rho = c(0.3),
...
N_reps <- 200
scenario <- expand.grid(nGroupT = c(10), nGroupC = 10, meanC = c(3), 
                        sdC = c(0.05, 0.5),
                        ef = c(0.25), rho = c(0.3),
                        sampleSize = c(5， 10, 15, 20, 30), 
                        idSD = c(0.01, 0.05, 0.1， 1),
                        B = 500,
                        N_reps = 1:N_reps)
...
save(final_results, file = 'results_final2.Rdata')
print(Sys.time() - t0)
...
result <- final_results %>%
  arrange(nGroupT, nGroupC, sdC, ef, idSD, sampleSize) %>% 
  group_by(nGroupT, nGroupC, sdC, ef, idSD, sampleSize) %>%
